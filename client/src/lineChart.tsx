...
const LineChart: React.FC<Properties> = (props: Properties) => {
  ...
  const colours: string[] = ['#C22D2D', '#0071B2', '#009E73', '#E69D00', '#CC79A7'];
  ...
  const addDerivativeSeries = (data: any, index: any): void => {
        ...
        if (index !== undefined) {
          let parentIndex: any = props.sensors.findIndex(item => item.name + "'" === index);
        ...
        lineSeries[lineSeries.length - 1]
            ...
            }).setFillStyle(solidfill => solidfill.setA((index !== undefined) ? parseInt('80') : parseInt('FF'))))
            .setMouseInteractions(mouseInteractions)
            .setResultTableFormatter((builder:any, series:any, Xvalue:any, Yvalue:any) => {
                return builder.addRow(series.getName() + ": " + Yvalue.toFixed(2) + " " + props.sensors[lineSeries.length - 1].output_unit)
            })
  ...
  const createChart = (): void => {
    //
        ...
        for (var i = 0; i < props.sensors.length; i++) {
            lineSeries.push(chart.addLineSeries({ dataPattern: DataPatterns.horizontalProgressive }).setName(props.sensors[i].name));
            lineSeries[i]
                .setStrokeStyle(new SolidLine({
                    thickness: 2,
                ...
                .setResultTableFormatter((builder: any, series: any, Xvalue: any, Yvalue: any) => {
                    return builder
                        .addRow(series.getName() + ": " + Yvalue.toFixed(2) + ' ' + props.sensors[0].output_unit)
                });
        }
    ...
    const changeInterval = (lower: number, upper: number): void => {chart.getDefaultAxisX().setInterval(lower, upper) }
    ...
    const toggleSeries = (index: any): void => {
                ...
                <div id='chart-text'>
                    <Card border='light' style={{ width: '313px', margin: 'auto' }}>
                        <Card.Body style={{ padding: '0' }}>
                            <div id='outer-col' className='col' style={{ textAlign: 'center', padding: '0', paddingBottom: '3px', margin: 'auto' }}>
                                <div className='row' style={{ textAlign: 'center', padding: '0', margin: 'auto' }}>
                                    <div style={{ margin: 'auto' }}>
                                        <div className='col-xs' style={{ color: colours[sensor], fontStyle: 'bold', textAlign: 'right', padding: '0', fontSize: '1rem', display: 'inline-block', marginTop: '5px', width: '143px' }}>
                                            <Button id='derivativeButton' onClick={() => {props.controlDerivative(sensors[sensor].name) }}><b style={{ fontStyle: 'italic', fontSize: '1rem' }}>f'(x)</b></Button>
                                            <b style={{ fontSize: '1rem', float: 'right', display: 'inline-block', marginTop: '3px', cursor: 'pointer', textDecoration: lineSeries[sensor] !== undefined && lineSeries[sensor].isDisposed() ? 'line-through': 'none' }} onClick={() => toggleSeries(sensor)}>{sensors[sensor].name}:</b>
                                        </div>
                                        <div className='col-xs' style={{ fontStyle: 'bold', textAlign: 'center', padding: '0', fontSize: '1rem', width: '70px', display: 'inline-block' }}><b style={{ verticalAlign: 'middle' }}>{(data === undefined) ? '0' : data[sensor]}</b></div>
                                        <div className='col-xs' style={{ fontStyle: 'bold', textAlign: 'left', padding: '0', fontSize: '1rem', display: 'inline-block', marginBottom: '5px' }}><b style={{ verticalAlign: 'middle', marginTop: '3px' }}>{sensors[0].output_unit}</b></div>
                                    </div>
                                </div>
                                {
                                    derivative.length !== 0 ?
                                        <div className='row' style={{ textAlign: 'center', padding: '0', margin: 'auto', marginTop: '5px', width: '100%' }}>
                                            <div style={{ margin: 'auto' }}>
                                                <div className='col-xs' style={{ color: colours[sensor] + '80', fontStyle: 'bold', textAlign: 'right', padding: '0', fontSize: '1rem', display: 'inline-block', width: '143px' }}><div style={{ width: '34px' }} /><b>{derivative[0].name}</b></div>
                                                <div className='col-xs' style={{ fontStyle: 'bold', textAlign: 'center', padding: '0', fontSize: '1rem', width: '70px', display: 'inline-block' }}><b></b></div>
                                                <div className='col-xs' style={{ fontStyle: 'bold', textAlign: 'left', padding: '0', fontSize: '1rem', width: '100px', display: 'inline-block', marginBottom: '5px' }}><b style={{ verticalAlign: 'middle', marginTop: '3px' }}></b></div>
                                            </div>
                                        </div>
                                        :
                                        null
        ...
        return (
            <div id='lineChart' style={{ marginBottom: '20px' }}>
                ...
                <div id={chartId.toString()} style={{ height: '500px' }} className='fill'></div>
            </div>
        );
};

...
const GraphBox: React.FC<Properties> = (props: Properties) => {
  ...
  const RangeSlider = withStyles({
    ...
    useEffect(() => {
        ...
        document.addEventListener('gotData', () => { pullData() });

        return () => {
          document.removeEventListener('gotData', () => { pullData() });
        }
    }, []);

    const pullData = (): void => {
        let newDatasets: Promise<any>;
        if (props.sensors[0].category !== 'Track Map') newDatasets = Data.getInstance().get(props.sensors[0].category);
        ...
        newDatasets.then((newDatasets1) => {
            ...
            if (props.title === 'Track Map') setData(newDatasets1);
                ...
                if (newDatasets1.length === undefined) newColour = updateColours(newDatasets1);
                else {
    ...
    const createDerivative = (parentIndex: any, sensor: any): void => {
        ...
        Data.getInstance().getAllData(sensor).then((data1: number[]) => {
            data1 = smoothenData(data1, 1, false);
            let dx = smoothenData(data1, 0.1, true);
            chart.current.addDerivativeSeries(dx, sensor + "'");
            setSensors(oldArray => [...oldArray, {
                derivative: true,
                name: sensor + "'",
                parent: sensor,
                output_unit: props.sensors[0].output_unit.trim() !== "" ? props.sensors[0].output_unit + "/sec" : props.sensors[0].output_unit
            }]);
    ...
    const removeDerivative = (parentIndex: number, sensor: any): void => {
        ...
        setSensors(oldSensors => oldSensors.filter(element => element.name !== sensor + "'"));
        setUpdatingTitles(true);
    }

    const controlDerivative = (sensor: any): void => {
        const parentIndex = props.sensors.findIndex(item => item.name === sensor);
        if (dxdtParentIndices.includes(parentIndex)) removeDerivative(parentIndex, sensor);
    ...
    const updateDerivative = async (): Promise<any> => {
        for (var i in dxdtParentIndices) {
            ...
            await Data.getInstance().getAllData(sensor).then((data1: number[]) => {
                data1 = smoothenData(data1, 1, false);
                let dx = smoothenData(data1, 0.1, true);
    ...
    const handleWindowChange = async (event: any, value: number) => {
        if (value !== window && value < index / 4) {
            for (var i in dxdtParentIndices) {
                const sensor = props.sensors[dxdtParentIndices[i]].name;
                await Data.getInstance().getAllData(sensor).then((data: number[]) => {
    ...
    if (sensors[0].category === 'Track Map') {
            return (
                <div id='graphBox'>
                    ...
                    {
                        !props.hideClose ?
                            <Button id='deleteGraph' onClick={() => props.delete(props.id)} style={{ position: 'absolute' }}>
            ...
            return (
                <div id='graphBox' style={{ borderColor: indicationColour }}>
                    <p id='graphTitle' style={{ height: '30px' }}><div style={{ paddingTop: '3.5px', paddingBottom: '3.5px' }}><b style={{ color: indicationColour }}>{props.sensors[0].category}</b></div></p>
                    ...
                    <div id='graphBottom' style={{ width: '50%', margin: 'auto' }}>
                        <Button id='toggleAxis' onClick={toggleRightAxis} style={{ position: 'absolute' }}>
                            <img id="logoImg" style={{ width: '26px', marginTop: '-11px', marginLeft: '-12px', position: 'absolute'}} src={require('../../../assets/rightAxis.svg')} />
                        </Button>
                        <Button id='toggleGrid' onClick={toggleGrid} style={{ position: 'absolute' }}>
